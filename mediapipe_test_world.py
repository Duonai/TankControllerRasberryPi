...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="MediaPipe Pose Landmarker world-coordinate elbow angle demo"
    )
    parser.add_argument("--model", default=default_model, help="Path to pose landmarker task model")
...
def annotate_elbow_angle_world(
    frame: np.ndarray,
    image_landmarks,
    world_landmarks,
    shoulder_idx: int,
    ...
    height, width = frame.shape[:2]
    shoulder_image = image_landmarks[shoulder_idx]
    elbow_image = image_landmarks[elbow_idx]
    wrist_image = image_landmarks[wrist_idx]

    if not all(landmark_visible(point) for point in (shoulder_image, elbow_image, wrist_image)):
        return None

    shoulder_world = world_landmarks[shoulder_idx]
    elbow_world = world_landmarks[elbow_idx]
    wrist_world = world_landmarks[wrist_idx]

    angle = angle_deg(
        (shoulder_world.x, shoulder_world.y, shoulder_world.z),
        (elbow_world.x, elbow_world.y, elbow_world.z),
        (wrist_world.x, wrist_world.y, wrist_world.z),
    )

    elbow_point = normalized_to_pixel(elbow_image, width, height)
        ...
        try:
            while True:
                ...
                if result.pose_landmarks and result.pose_world_landmarks:
                    ...
                    draw_upper_body(frame, image_landmarks)
                    left_angle = annotate_elbow_angle_world(
                        frame,
                        image_landmarks,
                        world_landmarks,
                        11,
                    ...
                    right_angle = annotate_elbow_angle_world(
                        frame,
                        image_landmarks,
                        world_landmarks,
                        12,
                ...
                cv2.imshow("MediaPipe Upper Body World", frame)
